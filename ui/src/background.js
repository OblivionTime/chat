...
ipcMain.on("open-window", (event, options) => {
  ...
  if (options.method == "audio") {
    new_win.loadURL(
      winURL +
        `audio?room=${room}&sender=${sender}&receiver=${receiver}&beInviter=${beInviter}`
    );
  } else if (options.method == "video") {
    new_win.loadURL(
      winURL +
        `video?room=${room}&sender=${sender}&receiver=${receiver}&beInviter=${beInviter}`
    );
  } else if (options.method == "group_audio") {
    new_win.loadURL(
      winURL +
        `group_audio?room=${room}&sender=${sender}&group_id=${options.group_id}&beInviter=${beInviter}`
    );
  } else if (options.method == "group_video") {
    new_win.loadURL(
      winURL +
        `group_video?room=${room}&sender=${sender}&group_id=${options.group_id}&beInviter=${beInviter}`
    );
  }
});
